...
export function collectCartData(cart) {
  return cart.map(item => {
    const el = document.querySelector(`.cart-list__item[data-id="${item.id}"]`);
    if (!el || item.removed) return null;

    return {
      id: item.id,
      size: el.querySelector(`input[name="size-${item.id}"]:checked`)?.value,
      color: el.querySelector(`input[name="color-${item.id}"]:checked`)?.value,
      qty: Number(el.querySelector('.cart-list__qty-input')?.value || 1)
    };
  }).filter(Boolean);
}
